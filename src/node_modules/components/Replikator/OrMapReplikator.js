import React, { Component, cloneElement, Children } from "react"
import replikativ, {
  hashIt,
  toEdn,
  createUUID,
  streamIntoIdentity,
  createStage,
  clientPeer,
  createOrMap,
  newMemStore,
  connect,
  associate
} from "replikativ";

// initialize replikativ
//const user = "mail:alice@stechuhr.de";
//const ormapId = createUUID("07f6aae2-2b46-4e44-bfd8-058d13977a8a");
//const uri = "ws://localhost:31778";



function wrapEvalFunctions(evalFns, component) {
  let wrappedFunctions = {};
  Iterator<?> keys = evalFns.keys();

  while( keys.hasNext() ) {
    String key = (String)keys.next();
    if ((typeof jObject.get(key)) === "function") {
      evalFunction = jObject.get(key);
      wrappedFunctions[key] = function(supervisor, old, params) {
        const result = evalFunction(supervisor, old, params);
        component.setState({atom: result});
        return result;
      }
    }
  }
  return wrappedFunctions;
}


function logError(err) {
  console.error(err);
}

function createReplica(userId, ormapId, uri, evalFns) {
  var replica = {
    atom: {},
    userId,
    ormapId,
    uri
  };
  // initialize local memory store
  newMemStore().then(function(store) {
    replica.store = store;
    // initialize client peer
    return clientPeer(store);
  }, logError).then(function(peer) {
    replica.peer = peer;
    // create local stage
    return createStage(userId, peer);
  }, logError).then(function(stage) {
    replica.stage = stage;
    // initialize local streaming
    replica.stream = streamIntoIdentity(stage, userId, ormapId, evalFns, replica.atom)
    // create
    return createOrMap(stage, {id: ormapId, description: "transactions"})
  }, logError).then(function () {
    return connect(replica.stage, uri);
  }, logError).then((result) => {
    console.info("Replica initialized!");
  }, logError);
  return replica;
}

class OrMapReplikator extends Component {
  constructor(props) {
    super(props);
    const {userId, ormapId, uri, evalFunctions} = props;
    const wrappedEvalFunctions = wrappedEvalFunctions(evalFunctions, this);
    const replica = createReplica(userId, ormapId, uri, wrappedEvalFunctions);

    this.state = { replica };
  }
  render() {
    const childProps = {atom: this.state.atom, replica: this.state.replica};
    return Children.map(this.props.children, child => cloneElement(child, childProps));
  }
}

export default OrMapReplikator
